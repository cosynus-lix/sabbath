...
def main():
    ...
    for size_system in [3,5,10,15,18]:
        mat_to_hyp_as_json_logger.info(f"Working on size {size_system}")
        ...
        for ind_mode in range(num_modes):
            mat_to_hyp_as_json_logger.info(f"Reformulating PI controller as closed loop for mode {ind_mode}...")
        ...
        with open(f'./hybrid_reformulation/reformulation_size_{size_system}.hyb', 'w', encoding='utf-8') as f:
            mat_to_hyp_as_json_logger.info(f"Exporting as .hyb file in ./hybrid_reformulation/reformulation_size_{size_system}.hyb...")
            json.dump(problem, f, ensure_ascii=False, indent=2)
        ...
        for index_mode in range(num_modes):
            ...
            with open(f"./single_mode_reformulation/mode_{index_mode}_size_{size_system}.invar", "w", encoding='utf-8') as f:
                mat_to_hyp_as_json_logger.info(f"Exporting mode {index_mode} as .invar file in ./single_mode_reformulation/mode_{index_mode}_size_{size_system}.invar...")
                json.dump([problem], f, ensure_ascii=False, indent=2)
