...
### LUDO END OF COPIED INITIALIZATION
...
verify_po_logger = logging.getLogger(__name__)
...
def handle_args():
    ...
    # LUDO: these are from verify_hs. We need less, probably.
    parser = argparse.ArgumentParser()
    ...
    # LUDO The following inputs are the same as for the Valu3s script. Is it possible to ask for them
    # only if the problem can be studied through these methods?

    # Choose model
    # parser.add_argument(
    #     '--size', dest='size', type=int, default=18,
    #     choices=[2, 3, 5, 10, 15, 18],
    #     help="Size of the model."
    # )
    # parser.add_argument(
    #     '--round', dest='round', action='store_true'
    # )
    # parser.add_argument(
    #     '-references', '--references', dest='references',
    #     help="Manually specify the references as list of floats (dimensions depends on the model): float, float, float, float, ..."
    # )
...
### LUDO START OF COPIED FUNCTIONS
...
def get_gas_lyapunov(dyn_sys, solver_function, gas_opts, num_info=None, mode=None):
    ...
    try:
        verify_po_logger.info("Searching a lyapunov function candidate...")
        ...
        raise e
        verify_po_logger.info("Error searching for the Lyapunov function")
        return None
    s1 = time.time()
    ...
    if not gas_opts.validate_lyapunov:
        ...
        verify_po_logger.warning("Skipping validation of a read Lyapunov function...")
        return V_m

    # Validate the lyapunov function
    verify_po_logger.info("Validating the Lyapunov function...")
    ...
    if res:
        ...
        verify_po_logger.info("The lyapunov function is not valid (SMT check failed)")
        return None
...
def compute_lyapunov(dyn_sys, solver_function, gas_opts, num_info=None, mode=None):
    ...
    if (stable_systems is None):
        verify_po_logger.info("Cannot find a stability point (or single point) found")
        return (False, None, None)
    elif len(stable_systems) > 1:
        verify_po_logger.info("Find multiple stable_systems points, ignore the system")
        return (False, None, None)
    else:
        ...
        if lyap is None:
            verify_po_logger.info("Cannot find lyapunov")
            return (False, None, None)
        else:
...
def find_level_set(config, lyap, switching_surface, beta,
    ...
    if k_init is not None:
        ...
        if solver.is_valid(cond_k):
            verify_po_logger.info("Validated numeric K * 0.999.")

            k_init_smt_gt = Real(myround(k_init * 1.001))
            cond_k = cond.substitute({k : k_init_smt_gt})
            if solver.is_valid(cond_k):
                verify_po_logger.warning("Numeric K is NOT close to the optimal value.")
            else:
                verify_po_logger.info("Numeric K is close to the optimal value.")

            if num_info:
                num_info.ks[mode] = k_init * 0.999
            return LE(lyap, k_init_smt)
        else:
            verify_po_logger.info("Invalid numeric K!! Going back to binary search.")

    # Check if (lyap <= k & switching_surface) -> beta for all k
    if not has_init_bound:
        verify_po_logger.info("Checking for validity")
        is_valid = solver.is_valid(cond)
    else:
        verify_po_logger.warning("Using user-provided initial lower/upper bounds")
    ...
    if (not has_init_bound and is_valid):
        ...
        verify_po_logger.warning("Conditions are valid for all k...")
        if num_info:
        ...
        verify_po_logger.info("Found a valid k...")
        ...
        while (TRUE() == solver2.get_value(GT(upper_bound - lower_bound, delta_smt))):
            ...
            verify_po_logger.info("Delta,lower,upper: %.2f,%.2f,%.2f" % (
        ...
        verify_po_logger.info("FOUND APPROX UPPER BOUND %f" % float(Fraction(lower_bound.serialize())))
        ...
        verify_po_logger.warning("No suitable K found!")
        if num_info:
...
def minimize_k(config, formula, f, k):
    ...
    verify_po_logger.debug("Minimizing...")
    ...
    while (solver.is_sat(And(f_bound, GE(f,k)))):
        ...
        verify_po_logger.debug("Found cex %.2f" % float(Fraction(k_val.serialize())))

        # Cex, remove from next search
        f_bound = And(formula, GE(k, k_val))

    verify_po_logger.debug("Found k close enough %.2f" % float(Fraction(k_val.serialize())))
    return True, k_val
...
def verify_stability_aux(config, dyn_systems, stable, lyap, switching_predicate, x):
    ...
    if (not x_in_m0):
        ...
        verify_po_logger.info("Swapping m0 with m1.")
    ...
    verify_po_logger.info("Found k0 = %.10f" % float(Fraction((k0_val.serialize()))))
    ...
    if (s0_in_m0):
        ...
        verify_po_logger.info("Checking if boundary with m0 is a differential invariant. (i.e. point does not cross switching surface)")
        stays_in_m0 = Implies(And(f0_le_k0, switching_surface), LT(sw_der_m0, Real(0)))
        if (solver.is_valid(stays_in_m0)):
            verify_po_logger.info("The point is stable")
            return Result.STABLE
        else:
            verify_po_logger.info("The point may cross the switching surface and may not be stable")

    if (s0_in_m0 and s1_in_m0):
        # Both equilibrium points are in m0, we prove that x stays in m0
        verify_po_logger.info("CASE A: m0 already in the mode containing the stable point.")
        ...
        if (s0_in_m0 and not s1_in_m0):
            verify_po_logger.info("CASE B: ss0 in m0 and ss1 in m1")
        else:
            verify_po_logger.info("CASE C: ss0 in m1 and ss1 in m1")
        ...
        if res:
            ...
            if (solver.is_valid(Implies(may_enter_m1, stays_in_m1))):
                ...
                verify_po_logger.info("The boundary of the level set of m1 and switch is not a differential invariant")
                return Result.UNKNOWN
        else:
            verify_po_logger.info("Did not find any suitable K1!")
            return Result.UNKNOWN

    elif (not s0_in_m0 and s1_in_m0):
        # Cannot be stable (m0 stabilizes in m1 and m1 stabilizes in m0).
        verify_po_logger.info("The point cannot be stable!")
...
def sample_stability_aux(config, dyn_systems, switching_predicate, gas_optss, simul_file):
    ...
    def generate_points_from_simulation(simul_file):
        verify_po_logger.info(f"Extracting points from simulation: {simul_file}")
        points = []
        import scipy.io
    ...
    for x in points:
        verify_po_logger.info(f"Try nr {tested_points_nr}... {x}")
        tested_points_nr += 1
        verified = verify_stability_aux(config, dyn_systems,
                                        stable, lyap,
                                        switching_predicate, x.get())
        if verified == Result.STABLE:
            verify_po_logger.info(f"Verified!")
            verified_points.append(x)
        if verified == Result.UNSTABLE:
            verify_po_logger.info(f"NOT stable!")
        if verified == Result.UNKNOWN:
            verify_po_logger.info(f"Unknown")

        if tested_points_nr % 10 == 0:
            verify_po_logger.info(
                f"...tried {tested_points_nr}, verified {len(verified_points)}"
            )

    verify_po_logger.critical(f"Tried with {tested_points_nr} points.")
    if not verified_points:
        verify_po_logger.error(f"Exhausted nr of attempts. NOT verified.")
    else:
        verify_po_logger.info(f"Verified {len(verified_points)} points.")
        for p in verified_points:
            verify_po_logger.debug(p.str_point())
...
def find_stability_assumption(config,
    ...
    if (s0_in_m0 and s1_in_m0):
        num_info.lies = [0, 0]
        # CASE 2.1
        verify_po_logger.info("CASE 2.1 - all stable in m0")
        ...
        num_info.lies = [0, 1]
        # CASE 2.2
        verify_po_logger.info("CASE 2.2 - m0 stable in m0, m1 stable in m1")
        ...
        verify_po_logger.warning("The stability point of m[0] is in m[1]!")
...
def main(args):
    ...
    verify_po_logger.info("Finding assumptions...")
    ...
    try:
        ...
        if synthesize and only is None:
            assumptions, lyap = find_stability_assumption(config,
                                                          ...
                                                          num_info,
                                                          input_num_info_file)
            verify_po_logger.info("Assumption computation completed...")

            verify_po_logger.info("Found an assumption.") #, assumptions.serialize())

            # DEBUG
            solver = config.solver_function()
            if (not solver.is_sat(assumptions)):
                verify_po_logger.info("WARNING: found inconsistent assumptions")
            else:
                verify_po_logger.info("WARNING: assumptions ARE consistent!")
    ...
    verify_po_logger.info("Serializing the results in %s..." % output_file_path)
    ...
    # ### LUDO This is done in verify_hs, I do not know if we need it
